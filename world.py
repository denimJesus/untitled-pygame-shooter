...
class World(pg.sprite.Sprite):
    ...
    def update(self):
        ...
        Very crude and quick, more of a simulation of level generation than
        actual, meaningful level generation
        """
        side = None
        if self.rect.centery > HEIGHT+700:
            side = 0
        elif self.rect.centerx < -700:
            side = 1
        elif self.rect.centery < -700:
            side = 2
        elif self.rect.centerx > WIDTH+700:
            side = 3
